...
原理，例：某CTF题目引用了1.2.61的fastjson包，并不是最新版，那么，一般大部分的可能出题人不会用新链去出题（有点昂贵），那么，大概率使用的payload class就在1.2.61版本后禁用的，那么，我的考虑就是，扫码所有class，只要在1.2.61以及之前的版本不存在，但是1.2.61版本后的黑名单存在，那么，这个class对应的jar包，就存在可利用的链。
